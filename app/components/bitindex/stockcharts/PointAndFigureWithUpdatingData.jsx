...
let {HistogramSeries, LineSeries, AreaSeries, PointAndFigureSeries} = ReStock.series;
let {MouseCoordinates, CurrentCoordinate} = ReStock.coordinates;
let {EdgeContainer, EdgeIndicator} = ReStock.coordinates;

let {StockscaleTransformer, PointAndFigureTransformer} = ReStock.transforms;
let {TooltipContainer, OHLCTooltip, MACDTooltip} = ReStock.tooltip;
let {XAxis, YAxis} = ReStock.axes;
let {SMA} = ReStock.indicator;
let {ChartWidthMixin} = ReStock.helper;

let pointAndFigureTransformOptions = {
  boxSize: 0.10
};
    ...
    return (
      <ChartCanvas ref='chartCanvas' width={this.state.width} height={370} margin={{
      ...
      }} initialDisplay={30} dataTransform={[
        {
          ...
          transform: PointAndFigureTransformer,
