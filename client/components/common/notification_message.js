...
const NotificationMessage = Component({
    ...
    addIcon(item) {
        return ( < i className = {
            this.types.messagesIcon[(item.icon ? item.icon : this.types.vitalyTypes[item.type])] + ' icon'
        } > < /i>)
    },
    render() {
        return ( < div className = {
                'ui' + (this.state.hidden ? ' hidden' : '') + ' small icon message' + (this.props.item.type ? this.types.messageAccent[this.types.vitalyTypes[this.props.item.type]] : '')
            } >
            ...
            < div className = 'content' > {
                this.props.item.title ? < div className = 'header' > {
