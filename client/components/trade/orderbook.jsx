...
const Orderbook = Component({
    ...
    getMeteorData: function() {
        return {
            ordersSell: OrderBookItems.find({
            ...
            ordersMax: OrderBookItems.findOne({
                pairId: this.props.pair._id
            }, {
                sort: {
                    amount: -1
    ...
    goBuySell(item, e) {
        Dispatcher.dispatch({
            actionType: 'BUY_SELL_AUTOCOMPLETE',
            data: {
    ...
    renderSellItems() {
        ...
        let max = this.data.ordersMax
            ? this.data.ordersMax.displayAmount()
            : 1;
        return this.data.ordersSell.map((item) => {
            ...
            return (
                <tr key={item._id} onClick={this.goBuySell.bind(this, item)} className={!item.animate
                    ? 'animate'
                    : ''}>
                    <td className='five wide'>
                        <div className='bignum left'>{amount[0]}</div>
                        <div className='bignum dot'>.</div>
                        <div className='bignum right'>
                            <span>{amount[1]}</span>
                            {nulls.substr(0, 7 - amount[1].length)}
                        </div>

                        <span className='leveler negative' style={{
                            width: weight + '%'
                        }}></span>
                    </td>
            ...
            return (
                <tr key={item._id} onClick={this.goBuySell.bind(this, item)} className={!item.animate
                    ...
                    <td className='five wide'>
                        ...
                        <span className='leveler positive' style={{
                            width: weight + '%'
                        }}></span>
                    </td>
        ...
        return (
            ...
            <tr className='ui white text opacity'>
                ...
                <td className='five wide right aligned green markered text'>
                    <span className='direction'>Higest
