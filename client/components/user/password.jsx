PasswordPage = React.createClass({
  ...
  newPassword(){
    ...
    Accounts.changePassword(old_pass, password, (err)=>{
      if (err) {
        Dispatcher.dispatch({ actionType: 'NEW_NOTIFICATION', payload: { message: {_id:'pass_not_changed',accent:'error',icon:'error',header:'Error!',content:err.message,timeout:3000} } });
      //  console.log(err.message)
        this.setState({errorMessage: err.message});
      } else {
        Dispatcher.dispatch({ actionType: 'NEW_NOTIFICATION', payload: { message: {_id:'pass_changed',accent:'accept',icon:'accept',header:'Your password changed!',timeout:3000} } });
