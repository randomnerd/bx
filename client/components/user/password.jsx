PasswordPage = React.createClass({
  ...
  newPassword(){
    ...
    Accounts.changePassword(old_pass, password, (err)=>{
      if (err) {
        Dispatcher.dispatch({ actionType: 'NEW_NOTIFICATION', payload: { message: {_id:'pass_not_changed',type:'error',icon:'error',title:'Error!',message:err.message,timeout:3000} } });
      //  console.log(err.message)
        this.setState({errorMessage: err.message});
      } else {
        Dispatcher.dispatch({ actionType: 'NEW_NOTIFICATION', payload: { message: {_id:'pass_changed',type:'accept',icon:'accept',title:'Your password changed!',timeout:3000} } });
