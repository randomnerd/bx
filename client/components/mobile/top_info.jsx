...
const TopInfo = Component({
    ...
    getMeteorData() {
        return {
            ...
            tradesBid: OrderBookItems.findOne({
                pairId: this.props.pair._id,
                buy: false
            }, {
                sort: {
            ...
            tradesAsk: OrderBookItems.findOne({
                pairId: this.props.pair._id,
                buy: true
            }, {
                sort: {
        ...
        return (
            <div className={clss}>
                <div className='item double'>
                    ...
                    <p>{this.props.pair.lastPrice
                            ? parseFloat(this.props.pair.lastPrice).toFixed(4)
                            : 0.0000}</p>
                </div>
                <div className='item double'>
                    <h4 className="ui header">Bid/Ask</h4>
                    <p>
                        <span className="red text">{this.data.tradesBid
                                ? parseFloat(this.data.tradesBid.displayPrice()).toFixed(4)
                                : 0.0000}
                        </span>/
                        <span className="green text">
                            {this.data.tradesAsk
                                ? parseFloat(this.data.tradesAsk.displayPrice()).toFixed(4)
                                : 0.0000}</span>
                    </p>
                </div>
                <div className='item double'>
                    ...
                    <p>
                        <span className="text">{this.data.tradesLo
                                ? parseFloat(this.data.tradesLo.displayPrice()).toFixed(4)
                                : 0.0000}
                        </span>-
                        <span className="text">
                            {this.data.tradesHi
                                ? parseFloat(this.data.tradesHi.displayPrice()).toFixed(4)
                                : 0.0000}</span>
                    </p>
                </div>
                <div className='item double'>
                    <h4 className="ui header">Volume</h4>
                    <p>{this.props.pair.dayVolume
                            ? parseFloat(this.props.pair.dayVolume).toFixed(4)
                            : 0.0000}</p>
                </div>
            </div>
        )
